...
| `steps/`                   | BDD step definitions (mapped to page objects/API)    |
...
| `npm run test:bdd`         | Run BDD tests (Gherkin feature files)                |
...
This project uses [playwright-bdd](https://github.com/vitalets/playwright-bdd) to convert Gherkin feature files into Playwright tests.

- **Feature files**: `features/*.feature` — business-readable scenarios
- **Step definitions**: `steps/*.steps.ts` — delegate to existing page objects, fixtures, and API utilities
- **Generated tests**: `features/generated/` (gitignored) — auto-generated by `bddgen`
...
The `bdd` project in `playwright.config.ts` discovers all generated tests. Classic `.spec.ts` files remain intact and run alongside BDD tests via `npm test`.
